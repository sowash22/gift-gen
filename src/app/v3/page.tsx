...
import { Sprout, Flower2, Gift as GiftIcon, X, ArrowLeft, Sun, Moon, Link as LinkIcon } from 'lucide-react';
...
export default function Home() {
  ...
  const generateGifts = async () => {
    ...
    try {
      const response = await fetch('/api/generate-gifts', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...form, description: description.trim() }) });
  ...
  const restart = () => {
    setStep(0); setForm({ recipient: '', occasion: '', vibe: [] }); setDescription(''); setGeneratedGifts([]); setShowResults(false); setActiveGift(null); setNoResults(false);
  };
  ...
  return (
    <div className={`min-h-screen w-full bg-emerald-50 dark:bg-gray-900 text-stone-800 dark:text-gray-200 transition-colors duration-300 ${nunito.className}`}>
      ...
      <header className="fixed top-0 left-0 right-0 p-4 px-8 flex justify-between items-center z-30">
        ...
        <button onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')} className="relative flex items-center justify-center w-10 h-10 rounded-full bg-white/50 dark:bg-gray-800/50 hover:bg-emerald-100/50 dark:hover:bg-gray-700/50 backdrop-blur-sm transition-colors">
          <div className="relative w-5 h-5">
            <Sun className="absolute inset-0 text-stone-800 dark:text-transparent scale-100 dark:scale-0 transition-all duration-300" />
        ...
        <AnimatePresence mode="wait">
          {isGenerating ? (
              ...
              {noResults ? (
                <div className="text-center m-auto">
                  ...
                  <div className="flex items-center justify-center gap-4">
                    <button onClick={generateGifts} className="px-6 py-3 font-bold rounded-full bg-green-500 text-white hover:scale-105 transition-transform">
                      Try Again
                    </button>
            ...
            <motion.div key="form" {...fadeAnim} className="w-full max-w-3xl text-center">
              <motion.div className="mb-12 h-24 flex items-center justify-center">
                <AnimatePresence mode="wait">
                  <motion.h2 key={step} {...fadeAnim} transition={{ duration: 0.4 }} className="text-3xl md:text-4xl font-bold">
              ...
              <div className="h-20 flex items-center justify-center gap-6">
                {step > 0 && (
                  <motion.button onClick={prevStep} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center gap-2 px-4 py-2 rounded-full font-semibold hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
                    <ArrowLeft className="w-5 h-5" /> Back
                  </motion.button>
                )}
                {isFinalStep && (
                  <motion.button onClick={generateGifts} disabled={isGenerating || !form.recipient || !form.occasion} className="px-8 py-4 text-xl font-bold rounded-full bg-gradient-to-r from-green-500 to-teal-500 text-white shadow-lg hover:scale-105 disabled:opacity-50 transition-all duration-300">
                    {isGenerating ? <div className="w-6 h-6 border-2 border-white border-t-transparent rounded-full animate-spin"></div> : 'Grow Gift Ideas'}
                  </motion.button>
                )}
              </div>
            </motion.div>
...
const GiftCard: FC<{ gift: Gift; onClick: () => void; }> = ({ gift, onClick }) => (
  <motion.div onClick={onClick} initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ type: 'spring' }} className="cursor-pointer w-full h-full bg-white dark:bg-gray-800 rounded-2xl shadow-md hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 overflow-hidden border border-gray-200 dark:border-gray-700">
    ...
    <div className="p-4">
      <h3 className="font-bold text-lg truncate text-stone-800 dark:text-white">{gift.name}</h3>
      <p className="text-sm text-stone-600 dark:text-gray-400">{gift.estimatedPrice || ''}</p>
    </div>
  </motion.div>
);
...
const GiftDetailModal: FC<{ gift: Gift; onClose: () => void; }> = ({ gift, onClose }) => (
  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 backdrop-blur-md z-50 flex items-center justify-center p-4" onClick={onClose}>
    <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} onClick={e => e.stopPropagation()} className="relative w-full max-w-lg bg-white dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden">
      ...
      <div className="p-6">
        <h3 className="text-2xl font-bold mb-2 text-stone-900 dark:text-white">{gift.name}</h3>
        <div className="flex flex-wrap gap-2 mb-4">
          {gift.tags?.map(tag => <span key={tag} className="px-2 py-1 text-xs font-medium bg-gray-100 dark:bg-gray-700 text-stone-700 dark:text-gray-200 rounded-full">{tag}</span>)}
        </div>
        <p className="text-stone-600 dark:text-gray-400 mb-4">{gift.description}</p>
        {gift.estimatedPrice && <div className="font-semibold text-xl mb-4 text-green-700 dark:text-green-400">{gift.estimatedPrice}</div>}
        {gift.links?.[0] && <a href={gift.links[0]} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 w-full p-3 rounded-lg font-semibold bg-green-600 text-white hover:bg-green-700 transition-colors"><LinkIcon className="w-4 h-4" /> View Product</a>}
      </div>
    </motion.div>
  </motion.div>
);