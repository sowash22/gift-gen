...
const Background = () => (
  <div className="absolute inset-0 z-0">
    ...
    <motion.div 
      ...
      className="absolute bottom-1/4 left-1/4 w-80 h-80 bg-blue-200/15 dark:bg-blue-500/10 rounded-full blur-2xl" 
    />
  </div>
);
  ...
  return (
    <div className={`min-h-screen w-full bg-gradient-to-br from-emerald-50 via-green-50 to-teal-50 dark:from-gray-900 dark:via-gray-900 dark:to-gray-800 text-gray-800 dark:text-gray-100 ${nunito.className}`}>
      ...
      <header className="fixed top-0 left-0 right-0 p-6 flex justify-between items-center z-30 backdrop-blur-sm bg-white/40 dark:bg-gray-900/40">
        <motion.div 
          className="flex items-center gap-3 font-bold text-xl cursor-pointer text-green-600 dark:text-green-400" 
          onClick={restart}
        ...
        <motion.button 
          ...
          whileTap={{ scale: 0.95 }}
          className="w-12 h-12 rounded-full bg-white/70 dark:bg-gray-800/70 backdrop-blur-sm border border-gray-200/50 dark:border-gray-700/50 flex items-center justify-center hover:bg-white/90 dark:hover:bg-gray-700/90 transition-all shadow-sm"
        >
            ...
            {steps.map((_, i) => (
              <motion.div
                ...
                className={`h-3 rounded-full transition-all duration-500 ${
                  i === step ? 'bg-green-500 w-8 shadow-lg' : i < step ? 'bg-green-300 w-3' : 'bg-gray-300 dark:bg-gray-600 w-3'
                }`}
                layoutId={`step-${i}`}
              />
            ))}
            ...
            >
              {noResults ? (
                <div className="text-center">
                  <div className="w-24 h-24 bg-gradient-to-br from-orange-400 to-pink-500 rounded-full flex items-center justify-center mx-auto mb-6 shadow-lg">
                  ...
                  <button 
                    onClick={restart} 
                    className="px-8 py-4 bg-gradient-to-r from-green-400 to-emerald-500 text-white rounded-full font-semibold hover:from-green-500 hover:to-emerald-600 transition-all shadow-lg"
                  >
                    Try Again 🌟
                  </button>
                </div>
                ...
                <>
                  ...
                  >
                    ...
                    <p className="text-gray-600 dark:text-gray-400 text-lg">
                      {generatedGifts.length} wonderful {generatedGifts.length === 1 ? 'option' : 'options'} just for you ✨
                    </p>
                  </motion.div>
                  ...
                  <div className="text-center">
                    <button 
                      onClick={restart} 
                      className="px-8 py-3 bg-white dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-700 rounded-full font-medium hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors shadow-sm"
                    >
            ...
            >
              <motion.div className="mb-16">
                ...
                {/* {step === 0 && (
                  ...
                  >
                    Let's find something they'll absolutely love! 💖
                  </motion.p>
                )} */}
              </motion.div>
              ...
              <div className="flex items-center justify-center gap-6">
                <motion.button 
                  ...
                  className="flex items-center justify-center w-14 h-14 rounded-full bg-white dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-700 disabled:opacity-30 hover:bg-gray-50 dark:hover:bg-gray-700 hover:border-gray-300 dark:hover:border-gray-600 transition-all shadow-sm"
                >
                ...
                {isFinalStep ? (
                  <motion.button 
                    ...
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
                    className="relative px-10 py-4 bg-gradient-to-r from-green-400 via-emerald-500 to-teal-500 text-white rounded-full font-semibold text-lg shadow-xl hover:shadow-2xl disabled:opacity-50 transition-all overflow-hidden"
                  >
                    <motion.div
                  ...
                  <motion.button 
                    ...
                    className="flex items-center justify-center w-14 h-14 rounded-full bg-gradient-to-r from-gray-700 to-gray-800 dark:from-gray-200 dark:to-gray-300 text-white dark:text-gray-800 disabled:opacity-30 hover:from-gray-600 hover:to-gray-700 dark:hover:from-gray-300 dark:hover:to-gray-400 transition-all shadow-lg"
                  >
...
const OptionPill: FC<{ label: string; isSelected: boolean; onClick: () => void; }> = ({ label, isSelected, onClick }) => (
  <motion.button 
    ...
    whileTap={{ scale: 0.97 }} 
    onClick={onClick} 
    className={`group relative px-6 py-4 text-lg font-medium rounded-full border-2 transition-all duration-200 shadow-sm hover:shadow-lg ${
      isSelected 
        ? 'bg-gradient-to-r from-green-400 to-emerald-500 border-green-500 text-white shadow-green-200 dark:shadow-green-900/50' 
        : 'bg-white/90 dark:bg-gray-800/90 border-gray-200 dark:border-gray-700 hover:border-green-300 dark:hover:border-green-600 hover:bg-green-50 dark:hover:bg-gray-700 backdrop-blur-sm'
    }`}
  >
...
const GiftCard: FC<{ gift: Gift; onClick: () => void; }> = ({ gift, onClick }) => (
  <motion.div 
    ...
    whileTap={{ scale: 0.98 }}
    className="group cursor-pointer bg-white dark:bg-gray-800 rounded-3xl shadow-lg hover:shadow-2xl border border-gray-100 dark:border-gray-700 overflow-hidden transition-all duration-300"
  >
    <div className="relative w-full aspect-square overflow-hidden bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-900">
      ...
      <motion.div
        className="absolute top-4 right-4 w-8 h-8 bg-white/90 dark:bg-gray-800/90 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 backdrop-blur-sm shadow-sm"
        initial={{ scale: 0 }}
        whileHover={{ scale: 1 }}
        transition={{ duration: 0.2 }}
      >
  ...
  >
    ...
    >
      <motion.button 
        ...
        whileTap={{ scale: 0.9 }}
        className="absolute top-4 right-4 z-10 w-10 h-10 rounded-full bg-white/95 dark:bg-gray-900/95 backdrop-blur-sm flex items-center justify-center hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors shadow-lg"
      >
        <X className="w-5 h-5 hover:text-red-500 transition-colors" />
      </motion.button>
      
      <div className="relative w-full aspect-video bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-800 dark:to-gray-700">
        <ImageWithFallback src={gift.images?.[0] || ''} alt={gift.name} />
      </div>
      
      <div className="p-6">
        <h3 className="text-2xl font-bold mb-3 text-gray-800 dark:text-gray-100 flex items-center gap-2">
          ...
          <div className="flex flex-wrap gap-2 mb-5">
            {gift.tags.map((tag, index) => (
              <motion.span 
                ...
                className="px-3 py-1 text-xs font-medium bg-gradient-to-r from-blue-100 to-purple-100 dark:from-blue-900/30 dark:to-purple-900/30 text-blue-700 dark:text-blue-300 rounded-full border border-blue-200 dark:border-blue-800"
              >
                {tag}
              </motion.span>
            ))}
        ...
        {gift.links?.[0] && (
          <motion.a 
            ...
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
            className="flex items-center justify-center gap-3 w-full p-4 bg-gradient-to-r from-green-400 to-emerald-500 text-white rounded-2xl font-semibold hover:from-green-500 hover:to-emerald-600 transition-all shadow-lg hover:shadow-xl"
          >
