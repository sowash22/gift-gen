...
export default function FeedbackModal({ isOpen, onClose }: FeedbackModalProps) {
  ...
  const positiveFeedbackOptions = ['like', 'love', 'unique', 'random', 'good', 'perfect', 'easy', 'fun', 'creative'];
  const negativeFeedbackOptions = ['bad', 'confusing', 'boring', 'repetitive', 'slow', 'wrong', 'difficult', 'plain'];
      ...
      <div className="bg-white/95 dark:bg-slate-800/95 backdrop-blur-xl rounded-3xl p-8 w-full max-w-2xl mx-4 shadow-3xl border border-white/30 dark:border-slate-700/50 transform animate-in">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <h2 className="text-3xl font-black text-slate-900 dark:text-white">Share Your Feedback</h2>
          </div>
          <button onClick={() => onClose({feedbackSubmitted: false})} className="p-3 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-2xl transition-all duration-300 transform hover:scale-110">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-4">
            <label className="text-lg font-bold text-slate-800 dark:text-slate-100">Tell us what you think</label>
          ...
          <textarea value={feedbackText} onChange={(e) => setFeedbackText(e.target.value)} placeholder="Share any thoughts, ideas, or issues to help us improve your experience." className="w-full px-6 py-6 border-0 outline-none rounded-3xl focus:ring-4 focus:ring-blue-500/20 bg-white/80 dark:bg-slate-800/80 backdrop-blur-md text-slate-900 dark:text-white placeholder-slate-500 dark:placeholder-slate-400 resize-none min-h-[120px] text-base leading-relaxed transition-all duration-300 shadow-xl shadow-blue-500/5 hover:shadow-2xl hover:shadow-blue-500/10 focus:shadow-2xl focus:shadow-blue-500/15 border border-white/20 dark:border-slate-700/50" rows={4} />
        </div>

        <div className="mb-8">
          <div className="flex flex-wrap gap-3">
            {positiveFeedbackOptions.map(option => (
              <button key={option} onClick={() => handlePositiveOptionToggle(option)} className={`px-4 py-3 rounded-2xl text-sm font-bold transition-all duration-300 transform hover:scale-105 shadow-lg border ${selectedPositiveOptions.includes(option) ? 'bg-gradient-to-r from-purple-500 to-indigo-500 text-white shadow-blue-500/30 border-blue-400/20' : 'bg-white/80 dark:bg-slate-800/80 backdrop-blur-md text-slate-700 dark:text-slate-300 shadow-slate-500/10 border-white/20 dark:border-slate-700/50'}`}>
                {option.charAt(0).toUpperCase() + option.slice(1)}
              </button>
            ))}
            {negativeFeedbackOptions.map(option => (
              <button key={option} onClick={() => handleNegativeOptionToggle(option)} className={`px-4 py-3 rounded-2xl text-sm font-bold transition-all duration-300 transform hover:scale-105 shadow-lg border ${selectedNegativeOptions.includes(option) ? 'bg-gradient-to-r from-red-400 to-pink-500 text-white shadow-red-500/30 border-red-400/20' : 'bg-white/80 dark:bg-slate-800/80 backdrop-blur-md text-slate-700 dark:text-slate-300 shadow-slate-500/10 border-white/20 dark:border-slate-700/50'}`}>
                {option.charAt(0).toUpperCase() + option.slice(1)}
              </button>
            ))}
          </div>
